...
function displayCurrentMenu(){
  if(displayMenu==1&&level!=0)
  {
    ...
    if(lost==true)
    {
      ...
      //ctx.fillText("REisStart",305,140);
    }
    else
    {
...
function displayAmmosAndLevel()
{
  ...
  // Deplacer le monstre
  deplacerLesMonstres();
  if(allPlayers[username].ammo>0 && level !=0 && displayMenu>=0)
  {
    ...
    var number ={};
    var str = allPlayers[username].ammo.toString();
    number = str.split("");
   // console.log(number);
    if(number.length == 1)
    {
...
function mouseDown(){
  ...
  if (inputStates.mousedown) {
    ...
    if(allPlayers[username].ammo>0 && cooldown==true){
      if(displayMenu==0)
      {
        if(allPlayers[username].x >=0 && allPlayers[username].x <=801){
          if(allPlayers[username].y>=0 && allPlayers[username].y <=460){
            if(displaySound==1){
              var sound = new Howl({
                urls: ['shoot.mp3']
              }).play();
            }
            allPlayers[username].ammo--;
            cooldown=false;
            if (testeCollisionJoueurMonstres(allPlayers[username])) {
              //player.color = 'red';
              //retourner le monstre qui a été toucher 
              var i = quelMonstreMort();
              ...
              {  
                ...
                socket.emit('sendDuckShot',i, tabMonstres[i].hp, tabMonstres[i].isDead);
              }
            }
          }
        }
    ...
    if(displayMenu == 1)
    {
      //isStart or next button
      if(allPlayers[username].x >= 320 && allPlayers[username].x <= 430)
      {
        if(allPlayers[username].y >= 320 && allPlayers[username].y <=340)
        {
          //console.log("clique sur le bouton next / isStart");
          if(lost==true){
            level=1;
            lost=false;
          }
          ...
          tabMonstres = [];
          resetTab=true;
          socket.emit('sendStart', level, isStart, displayMenu, countDeads);
        }
        if(allPlayers[username].y >=400 && allPlayers[username].y <=415)
        {
    ...
    if(displayMenu==2)
    {
      if(allPlayers[username].x >= 330 && allPlayers[username].x <= 420)
      {
        if(allPlayers[username].y >= 440 && allPlayers[username].y <=460)
        {
          displayMenu=1;
        }
      }
    ...
    if(allPlayers[username].x>=80 && allPlayers[username].x<=145)
    {
      if(allPlayers[username].y>=480 && allPlayers[username].y<=500)
      {
        if(cooldown==true)
        {
          cooldown=false;
