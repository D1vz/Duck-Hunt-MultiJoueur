...
io.sockets.on('connection', function (socket) {
	...
	socket.on('sendgame', function (username,lvl,deads,isLost,menu,starting)
	{
		console.log(username+" : "+lvl+","+deads+","+isLost+","+menu+","+starting);
		socket.broadcast.emit('updategame', lvl, deads,isLost,menu,starting);
	});

	socket.on('sendStart', function (lvl,starting,menu,deads)
	{
		//console.log("sendStart : "+lvl+","+starting+","+menu+","+deads);
		socket.broadcast.emit('startGame', lvl, starting,menu,deads);
	});

	socket.on('sendDuckShot', function (duckShot, duckHP, duckIsDead)
	{
		console.log("sendDuckShot");
		socket.broadcast.emit('updateDucks', duckShot, duckHP,duckIsDead);
	});
	...
	socket.on('adduser', function(username){
		...
		//console.log("usernames.length" + Object.keys(usernames).length);
		...
		var player = {
		  ...
		  color: 'black',
		  ammo:10,
		  numPlayer:Object.keys(usernames).length
		}
